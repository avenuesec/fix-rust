...
pub struct ExecutionReportFields {
    ...
    pub current_cost_basis : Option<f32>, // FIELD_CURRENTCOSTBASIS 8005
    pub cancel_open_qty : Option<f32>, // FIELD_CANCELOPENQTY 9310
...
pub struct NewOrderSingleFields {
    ...
    pub clearing_firm : Option<String>, // FIELD_CLEARINGFIRM 439
    pub clearing_account : Option<String>, // FIELD_CLEARINGACCOUNT 440
    pub trader_identifier : Option<String>, // FIELD_TRADERIDENTIFIER 9008
    pub cl_ref_request_id : Option<String>, // FIELD_CLREFREQUESTID 9203
...
const FIELD_CURRENTCOSTBASIS : u32 = 8005; // AMT


const FIELD_TRADERIDENTIFIER : u32 = 9008; // STRING


const FIELD_CLREFREQUESTID : u32 = 9203; // STRING


const FIELD_CANCELOPENQTY : u32 = 9310; // FLOAT
...
fn parse_message_execution_report_fields( consumer : &mut FixConsumer  ) -> ExecutionReportFields {
    ...
    let mut current_cost_basis : Option<f32> = None;
    let mut cancel_open_qty : Option<f32> = None;
    ...
    while let Some(fld) = consumer.next() {
        match fld {
            ...
            &FieldVal { id: FIELD_CURRENTCOSTBASIS, val: v } => {

                current_cost_basis = Some( f32::from_str(v).unwrap() );
            },
            &FieldVal { id: FIELD_CANCELOPENQTY, val: v } => {

                cancel_open_qty = Some( f32::from_str(v).unwrap() );
            },
            _ => {
                // unknown field? why? should it be added raw to some list?
            }
    ...
    ExecutionReportFields {
        ...
        cancel_open_qty: cancel_open_qty,
    }
}
...
fn parse_message_new_order_single_fields( consumer : &mut FixConsumer  ) -> NewOrderSingleFields {
    ...
    let mut clearing_firm : Option<String> = None;
    let mut clearing_account : Option<String> = None;
    let mut trader_identifier : Option<String> = None;
    let mut cl_ref_request_id : Option<String> = None;
    ...
    while let Some(fld) = consumer.next() {
        match fld {
            ...
            &FieldVal { id: FIELD_TRADERIDENTIFIER, val: v } => {

                trader_identifier = Some( v.to_string() );
            },
            &FieldVal { id: FIELD_CLREFREQUESTID, val: v } => {

                cl_ref_request_id = Some( v.to_string() );
            },
            _ => {
                // unknown field? why? should it be added raw to some list?
            }
    ...
    NewOrderSingleFields {
        ...
        cl_ref_request_id: cl_ref_request_id,
    }
}
...
fn write_execution_report_fields(flds: &ExecutionReportFields, output: &mut Write) -> Result<(), io::Error> {
    ...
    if flds.current_cost_basis.is_some() {
        let val = flds.current_cost_basis.as_ref().unwrap();

        write!(output, "8005={}\u{01}", val )?; // FIELD_CURRENTCOSTBASIS
    }
    if flds.cancel_open_qty.is_some() {
        let val = flds.cancel_open_qty.as_ref().unwrap();

        write!(output, "9310={}\u{01}", val )?; // FIELD_CANCELOPENQTY
    }
    Ok( () )
}
...
fn write_new_order_single_fields(flds: &NewOrderSingleFields, output: &mut Write) -> Result<(), io::Error> {
    ...
    if flds.trader_identifier.is_some() {
        let val = flds.trader_identifier.as_ref().unwrap();

        write!(output, "9008={}\u{01}", val )?; // FIELD_TRADERIDENTIFIER
    }
    if flds.cl_ref_request_id.is_some() {
        let val = flds.cl_ref_request_id.as_ref().unwrap();

        write!(output, "9203={}\u{01}", val )?; // FIELD_CLREFREQUESTID
    }
    Ok( () )
}
